...
export function useFinchState() {
  ...
  const bootstrap=useCallback(async()=>{const requestSequence=++bootstrapSequence.current;const res=await fetch("/api/finch/tenants",{cache:"no-store"});const body=await res.json().catch(()=>({}));if(!res.ok)throw new Error(body.error||`HTTP ${res.status}`);if(requestSequence===bootstrapSequence.current){verificationBlocked.current=body.needsVerifiedEmail===true;if(alive.current)setTenants(body);}return body as TenantsResponse;},[]);
  const refetch=useCallback(async()=>{const requestSequence=++stateSequence.current;if(verificationBlocked.current){if(alive.current){setError(null);setLoading(false);}return;}try{const res=await fetch("/api/finch/state",{headers:{accept:"application/json"},cache:"no-store"});const body=await res.json().catch(()=>({}));if(requestSequence!==stateSequence.current)return;if(!res.ok){if(res.status===403){const refreshed=await bootstrap();if(requestSequence!==stateSequence.current)return;const personal=refreshed.tenants.find(t=>t.kind==="personal"&&t.state==="active")?.tenantId;if(personal){const selected=await fetch("/api/finch/tenants/select",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({tenantId:personal})});if(requestSequence!==stateSequence.current)return;if(selected.ok){window.location.reload();return;}}}throw new Error(body.error||`HTTP ${res.status}`);}if(alive.current){setState(body);setError(null);}}catch(e){if(requestSequence===stateSequence.current&&alive.current)setError(e instanceof Error?e.message:"failed to load state");}finally{if(requestSequence===stateSequence.current&&alive.current)setLoading(false);}},[bootstrap]);
